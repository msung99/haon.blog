...
> 💡 현재 포스트는 [하모니 팀 기술 블로그](https://kakaotech-harmony.netlify.app/backend/local-cache-custom/) 에 게시된 글 입니다.
...
정말 좋은 기능이지만, 말만 들어도 성능상의 이슈가 발생할 것 같다. 로컬에서 테스트한 결과, 트래픽이 급증하는 상황에서 AI 모델 서빙 서버에 한번 API 요청을 보내고 응답 받기까지 무려 **1,300ms 가 넘는 시간이 소요**된다. 이는 사용자 경험 측면에서 좋지 않는 상황이다.
